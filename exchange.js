...
for (i = 0; i <= 7; i++) {
    document.getElementById(currencySymbols[i]).innerHTML=currency[i];
}
   ...
   numberLength(8,15)
    for (i = 0; i <= 7; i++) {
       if (currency[i+8] < currency[i]) {
          document.getElementById(currencySymbols[i]).innerHTML=currency[i+8];
          document.getElementById(Signs[i]).innerHTML="\u2193"; //low
       } else if (currency[i+8] == currency[i]) {
          document.getElementById(currencySymbols[i]).innerHTML=currency[i+8];
          document.getElementById(Signs[i]).innerHTML="\u2500"; //no change
       } else {
          document.getElementById(currencySymbols[i]).innerHTML=currency[i+8];
          document.getElementById(Signs[i]).innerHTML="\u2191"; //high
       } 
    }
